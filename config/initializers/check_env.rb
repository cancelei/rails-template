  ...
  class << self
    ...
    def verify_activerecord_encryption_secrets
      ...
      DB_ENCRYPTION_ENV_VAR_NAMES.each do |env_var_name|
        if example_env_contents.include?(ENV.fetch(env_var_name))
          raise "#{env_var_name} is unchanged from example.env. Generate a new one with `bundle exec rails db:encryption:init`"
        end
      end
    end
  end
